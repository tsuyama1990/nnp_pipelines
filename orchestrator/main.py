...
class AppBuilder:
    ...
    def build(self) -> AppContext:
        ...
        factory = ComponentFactory(self.config, self.config_path, self.meta_config_path)
...
def main():
    ...
    if not initial_pot_path.exists():
        ...
        try:
            seed_gen = SeedGenerator(config, config_path, meta_path)
            seed_gen.run()
    ...
    app_context = AppBuilder(config, config_path, meta_path).build()
